...
scores_con = create_engine("oracle+cx_oracle://fico:fico@" + scores_tns)
...
def one_credit_scores():
    ...
    for lmt in list(range(1000000, 7000000, 1000000)):
        lmt = str(lmt)
        one_fn = str(base_file_name) + '_{}_1M.txt'.format(lmt[0])
...
# todo one_credit_scores()


def others_credit_scores():
    # others_fn = str(base_file_name) + 'others.txt'
    x = 0
    ...
    for x in list(range(2, 5)):
        ...
        others_fn = str(base_file_name) + '_1_{}A.txt'.format(x)
        with open(others_fn, 'a') as others_fh:
