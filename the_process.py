...
@profile
def initialize_files(the_files, mdjlog=None):
    file = the_files[0] if isinstance(the_files, list) else the_files
    # mdjlog = get_logger(the_files[0].split('_')[0]) if isinstance(
    #     the_files, list) else get_logger(the_files.split('_')[0])
    try:
...
def init1file(file, i=None, c=None):
    file, mdjlog, start = file, get_logger(file.lower().split('_')[0]), datetime.datetime.now()
...
def get_dpid(fl_dtls, mdjlog=None, plf=None):
    mdjlog = get_logger(fl_dtls['dp_name'])
...
def fp_router(file_id):
    try:
        ...
        if dpid:
            ...
            route_file(fl_dtls, mdjlog=mdjlog)
            route_filed_data(fl_dtls, mdjlog=mdjlog)
        else:
            mdjlog.error('\tPlease maintain institution details for: {}'.format(fl_dtls['dp_name']))
