...
dir2watch = drop_zone = '/IoC/SB2log/'
mdjlog = get_logger('SB2Zipp3DLogs')
    ...
    try:
        ...
        if not dtls_df.empty:
            ...
            try:
                if not dtls_df.empty:
                    imp_pool = ThreadPool()
                    dtls_df['file_name'], dtls_df['dp_name'], dtls_df['category'] = fn, dp_name, category
                    dpid, dtls_df['process_date'], dtls_df['stage'] = fn.strip().split('-')[0], process_date, stage
                    dtls_df['dpid'], dtls_df['yyyymm'] = dpid, process_date.strftime('%Y%m')
                    # collecxn.insert_many(dtls_df.to_dict('records'))
                    imp_pool.map(collect_error, dtls_df.to_dict('records'))
...
def zip_log_pro(zipp3DD, dir2watch, i):
    for zip_log, c in watch(dir2watch):
...
def report_selected_fields(dpname, dpid, yyyymm, file_id):
    all_stages_csr = collecxn.find({"dpid": dpid, "yyyymm": yyyymm})
...
def summarize_logs(all_stages_df, dpid, dpname, file_id, insttxn_smmry):
    try:
        ...
        #
        report_writer.save()
        mdjlog.info('analyzes completed for {} {} submission. Thank you! Again!'.format(dpname, monyyyy))
    except Exception as e:
        mdjlog.error(e)
