...
from IoCEngine.cores import mpcores, ppns
...
def syndidata(crdt_data, sbjt_data, meta_data, dp_meta, datCat, hdr, sb2file):
    ...
    try:
        ...
        if crdt_data.shape[0] > 5000:
            crdt_syndi_chunks = np.array_split(crdt_data, mpcores)
            ...
            for crdt_s_chunk in crdt_syndi_chunks:
                ...
                if sbjt_s_data is not None or not sbjt_s_data.empty:
                    syndi_iter.append(
                        (crdt_s_chunk, fac_sgmnt, iff_fac_cols, iff_sbjt_cols, meta_data, sb2file, sbjt_s_data, sbjt_sgmnt))
            with futures.ProcessPoolExecutor() as executor:
                syndi_rez = executor.map(syndi_pairs, syndi_iter)
        else:
            syndi_rez = [syndi_pairs((crdt_data, fac_sgmnt, iff_fac_cols, iff_sbjt_cols, meta_data, sb2file, sbjt_data, sbjt_sgmnt))]
...
def syndi_pairs(targs):
    crdt_data, fac_sgmnt, iff_fac_cols, iff_sbjt_cols, meta_data, sb2file, sbjt_data, sbjt_sgmnt = \
        targs[0], targs[1], targs[2], targs[3], targs[4], targs[5], targs[6], targs[7]
    dp_name, syndicaxn_complt, syndi_data_list = meta_data['dp_name'], None, []
    mdjlog, dataCount, cust_id, ac_no = get_logger(dp_name), 0, None, None
    ...
    mdjlog.info("Syndicating {} data file {} & counting @#{}".format(dp_name, sb2file, dataCount))
    ...
    try:
        ...
        if parts > 1:
            ...
            data_list_hdr = '|'.join(
                ['HDHD', dpid, submxn_pt, d8reported, process_time, process_date, ctgry_dtls[sgmnt][1]])
            mdjlog.info("about to . .. syndicate {}".format(data_list_fn))
            syndidata(crdt_data, sbjt_data, meta_data, dp_meta, datCat, data_list_hdr,
                      data_list_fn)
