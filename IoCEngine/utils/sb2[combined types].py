...
from IoCEngine.commons import count_down, get_logger, mk_dir, right_now, std_out
...
def syndi_pairs(targs):
    ...
    bulk_upd8_data, i_type = [], 'corporate_submissions' if dataCat not in dataCatCheck else 'individual_submissions'
    ...
    # with progressbar.ProgressBar(max_value=crdt_shape) as bar:
    # bar = progressbar.ProgressBar(max_value=crdt_shape)
    # for idx in bar(crdt_data.index):
            ...
            if str(ac_no).replace('_', '').replace('-', '').strip() != '':
                if not str(ac_no).replace('_', '').strip().replace('-', '').strip().replace(' ', '').strip().isalpha():
                    ...
                    if not subj.empty:
                        ...
                        try:
                            branch_code = subj['branch_code'][0][-4:] if not isinstance(
                                subj.branch_code, str) else subj.branch_code[-4:]
                        except Exception as e:
                            branch_code = subj.branch_code.values[0][-4:] if not isinstance(
                                subj.branch_code, str) else subj.branch_code[-4:]
                        ...
                        ndx_line = {'_index': es_i, '_op_type': 'update', '_type': 'submissions',
                                    '_id': "-".join((dpid, ac_no2use, str(cycle_ver))), 'doc': data_line}
    ...
    try:
        helpers.bulk(es, bulk_upd8_data, ignore=[400, 404])
    except Exception as e:
        pass
        # mdjlog.error(e)
...
def log_syndi_pro(crdt_shape, dataCount, mdjlog, sb2file):
    if crdt_shape >= 5000:
        if not dataCount % 5000:
            mdjlog.info("Syndicating data file {} & counting @#{}_of_{}".format(sb2file, dataCount, crdt_shape))
    elif crdt_shape >= 500 and crdt_shape < 5000:
        if not dataCount % 1000:
            mdjlog.info("Syndicating data file {} & counting @#{}_of_{}".format(sb2file, dataCount, crdt_shape))
    elif crdt_shape < 500:
        if not dataCount % 150:
            mdjlog.info("Syndicating data file {} & counting @#{}_of_{}".format(sb2file, dataCount, crdt_shape))
        ...
        if data_size > chunk_size:
            ...
            for idx, crdt_data_chunk in enumerate(crdt_data_chunks):
                try:
                    ...
                    mdjlog.info(
                        "\n\nsyndicating CHUNK. .. \n\t{} credits with \n\t{} subjects".format(
                            crdt_data_chunk.shape,
                            sbjt_df.shape))
                    data_list_fn, data_list_hdr = syndi_params(ctgry_dtls, d8reported, dpid, mdjlog, sgmnt,
                                                               submxn_pt)
                    ...
                    size_done += size2use
                    mdjlog.info(
                        "\nQueuinG! #{} of {} Counts: {} of {} of {}".format(idx, rcount, size2use, size_done,
                                                                             data_size))
                    count_down(None, 1)
                except Exception as e:
                    mdjlog.error(e)
