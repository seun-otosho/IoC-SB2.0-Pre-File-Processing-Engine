...
def combo_data(dpid, loaded_batch, xtrcxn_zone):
    ...
    try:
        combo_cursor = pym_db.combined_submissions.find(
            {
                "dpid": dpid, "cycle_ver": loaded_batch['cycle_ver'], "status": "Loaded"
            })  # "batch_no": loaded_batch['batch_no'],
        combo_df = pd.DataFrame(list(combo_cursor))
        combo_df.fillna('', inplace=True)
        if not combo_df.empty:
            del combo_df['_id'], combo_df['batch_no'], combo_df['cycle_ver'], combo_df['dpid']
...
def fac_data(dpid, loaded_batch, xtrcxn_zone):
    mdjlog = get_logger(loaded_batch['dp_name'])
    try:
        fac_cursor = pym_db.facility_submissions.find(
            {
                "dpid": dpid, "cycle_ver": loaded_batch['cycle_ver'], "status": "Loaded"
            })  # "batch_no": loaded_batch['batch_no'],
        ...
        if not fac_df.empty:
            del fac_df['_id'], fac_df['batch_no'], fac_df['cycle_ver'], fac_df['dpid']
...
def corp_data(dpid, loaded_batch, xtrcxn_zone):
    ...
    try:
        ...
        if not corp_df.empty:
            del corp_df['_id'], corp_df['batch_no'], corp_df['cycle_ver'], corp_df['dpid']
...
def ndvdl_data(dpid, loaded_batch, xtrcxn_zone):
    mdjlog = get_logger(loaded_batch['dp_name'])
    try:
        ndvdl_cursor = pym_db.individual_submissions.find(
            {"dpid": dpid, "cycle_ver": loaded_batch['cycle_ver'], "status": "Loaded"}
        )  # "batch_no": loaded_batch['batch_no'],
        ndvdl_df = pd.DataFrame(list(ndvdl_cursor))
        ndvdl_df.fillna('', inplace=True)
        if not ndvdl_df.empty:
            del ndvdl_df['_id'], ndvdl_df['batch_no'], ndvdl_df['cycle_ver'], ndvdl_df['dpid']
